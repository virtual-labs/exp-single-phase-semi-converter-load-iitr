  ...
  if (
    ...
    Plotly.newPlot(
      ...
      ],
      {
        ...
        yaxis: {
          range: [
            -1 * (wave_forms[0][1] + 0.1 * wave_forms[0][1]),
            wave_forms[0][1] + 0.1 * wave_forms[0][1],
          ],
          title: "<b>Current(A)</b>",
          fixedrange: true,
        },
      ...
      ],
      {
        ...
        yaxis: {
          range: [
            ...
            wave_forms[0][1] + 0.1 * wave_forms[0][2],
          ],
          title: "<b>Current(A)</b>",
          fixedrange: true,
        },
...
function generategraphfinal() {
  ...
  const inductance = values["L1"]["value"] * 0.001;
  var z = Math.sqrt(
  ...
  var locurr = [],
    ...
    iavg_use = 0,
    irms_use = 0;
  const reavar = `alpha${sta1}r${loadResistance}l${inductance * 1000}`;
  console.log(data[reavar]);
  for (let i = 0; i < 600; i++) {
    locurr.push(data[reavar][0][i] * parseInt(sineamplitude));
    freecurr.push(data[reavar][1][i] * parseInt(sineamplitude));
    iavg_use += parseFloat(data[reavar][0][i] * parseInt(sineamplitude));
    irms_use += parseFloat(
      data[reavar][0][i] *
        parseInt(sineamplitude) *
        data[reavar][0][i] *
        parseInt(sineamplitude)
    );
    
    if (data[reavar][0][i] * parseInt(sineamplitude) > max_load_current) {
      max_load_current = data[reavar][0][i] * parseInt(sineamplitude);
    }
    if (data[reavar][1][i] * parseInt(sineamplitude) > max_free_current) {
      max_free_current = data[reavar][1][i] * parseInt(sineamplitude);
    }
  }
  ...
  irms = Math.sqrt(irms_use / input_voltage.length);
  if (vrms < 1) {
  ...
  return [
    [sineamplitude, max_load_current, max_free_current],
    [input_voltage, xval],
    [gate_pulse1, xval],
    [gate_pulse2, xval],
    [load_voltage, xval],
